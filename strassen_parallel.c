...
void strassen_parallel (int size, int *a, int *b, int *c)
{
	int *m1, *m2, *m3, *m4, *m5, *m6, *m7, *a11, *a12, *a21, *a22,
	    *b11, *b12, *b21, *b22, *c11, *c12, *c21, *c22, *temp1, *temp2,
	...
	/* combine matrix */
	int i, j;
	#pragma omp parallel for private(j) num_threads(OMP)
	for (i = 0 ; i < newSize ; i++) {
		for (j = 0 ; j < newSize ; j++) {
			c[i * size + j] = c11[i * newSize + j];
			c[newSize + i * size + j] = c12[i * newSize + j];
...
int main (int argc, char **argv)
{
	...
	strassen_parallel(input_size, a, b, c);
