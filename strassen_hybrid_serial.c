...
int main (int argc, char **argv)
{
	...
	printf("hybrid strassen serial   : %f sec\n",diff_in_second(start, end));

	FILE *fp_out = fopen("strassen_hybrid_serial.txt", "w");
	for(i = 0 ; i < tmp ; i++) {
		for(j = 0 ; j < tmp ; j++) {
			if(i < a_row && j < a_col)
